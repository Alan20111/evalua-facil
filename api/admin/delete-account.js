...
// Elimina definitivamente la cuenta de un docente desde el panel de admin.
// Misma limpieza que api/account/delete.js (docente que se da de baja solo),
// pero el que ejecuta es el admin, no el propio docente. Se agrega
// `eliminadoPorAdmin` en la constancia de baja para trazabilidad.
//
// Diferencias clave respecto al autoservicio:
//   · Se verifica que el LLAMANTE sea admin (no la víctima).
//   · No exige palabra de confirmación — el modal del panel ya la pidió.
//   · Llama revokeRefreshTokens antes de empezar: el docente no puede obtener
//     nuevos tokens mientras dure la limpieza. Los tokens ya emitidos siguen
//     válidos hasta 1 h (límite de Firebase), pero users/{uid} se borra en el
//     paso 6 y las reglas de Firestore fallan al buscar el rol desde ese momento.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Invalida los refresh tokens del docente de inmediato: no puede obtener
    // nuevos access tokens. Los ya emitidos expiran solos en ≤ 1 h.
    ...
    // ── 4. Lo que se identifica con el uid ──────────────────────────────
