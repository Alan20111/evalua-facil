...
- **B. Fuentes por parcial** — sección independiente por parcial (Parcial 1,
  Parcial 2, Parcial 3…) para fuentes específicas de ese periodo. **Uso
  real, vigente:** alimenta las operaciones normales de IA del semestre
  (crear examen/cuestionario OP-03/04, crear actividad OP-05, rúbrica OP-06,
  cotejo OP-07, reactivos OP-09) vía `bloqueFuentesPermanentes` en
  `functions/ia.js` — generales + las del parcial de la actividad. **NO
  alimenta** Diagnóstico del grupo ni Planeación Didáctica Inicial (13-ago-2026,
  ver changelog): esos dos apartados ocurren UNA sola vez al arrancar el
  curso, antes de que exista ningún parcial cursado, así que las fuentes de
  parciales futuros no tienen nada que aportarles todavía.
...
7. Fuentes específicas por parcial (solo para las operaciones normales del
   semestre — OP-03/04/05/06/07/09 — NO para Diagnóstico ni Planeación
   Inicial, ver Apartado 1 y changelog 13-ago-2026)
...
| 13-ago-2026 | **Se quita "Fuentes por parcial" de la UI de Config Asistente IA — CORREGIDO el mismo día, ver siguiente entrada.** Kike señala que no recuerda haberla pedido; investigación inicial (incompleta) reportó que ningún código leía `fuentesAsignatura` con `ubicacion:'parcial'`, así que se quitó la sección de `AsistenteIATab.jsx`. **Este hallazgo resultó estar mal** — ver la entrada de corrección inmediatamente abajo. |
| 13-ago-2026 | **CORRECCIÓN el mismo día: se restaura "Fuentes por parcial".** La investigación que llevó a quitarla (entrada anterior) solo revisó `src/`, no `functions/`: `bloqueFuentesPermanentes` (`functions/ia.js`) SÍ lee `fuentesAsignatura` con `ubicacion:'parcial'` y alimenta las operaciones normales del semestre — OP-03/04 (crear examen/cuestionario), OP-05 (crear actividad), OP-06 (rúbrica), OP-07 (cotejo), OP-09 (reactivos) — es la biblioteca de "el material específico de este parcial" para todo lo que un docente genera con IA durante el curso, no algo exclusivo de Planeación Inicial. Kike confirma restaurarla. Queda así: **Fuentes por parcial SIGUE VIGENTE** para las operaciones normales del semestre; **Planeación Didáctica Inicial y Diagnóstico del grupo NO la usan** (decisión aparte, correcta y ya aplicada en `functions/ia.js`: esos dos apartados ocurren una sola vez al arrancar el curso, antes de que exista ningún parcial cursado, así que fuentes de parciales futuros no les sirven de nada). Sin cambios a tarifas, créditos ni modelos. |
