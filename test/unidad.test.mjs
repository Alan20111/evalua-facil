...
// ═══ Calificar con IA — la IA nunca evalúa el tiempo de entrega ══════════════
grupo('Calificar con IA — criterios de tiempo reservados al docente')

const { _pruebas: CAP } = require('../functions/calificarAplicar.js')
const NIV_T = [{ nombre: 'Excelente' }, { nombre: 'Bueno' }, { nombre: 'Satisfactorio' }, { nombre: 'Insuficiente' }]
const critT = (nombre, descriptores = ['a', 'b', 'c', 'd'], puntos = [5, 3, 2, 0]) => ({ nombre, descriptores, puntos })
const R_NORMAL = { tipo: 'rubrica', niveles: NIV_T, criterios: [critT('Composición de dispositivos', ['Red completa', 'Casi', 'Incompleta', 'Nada']), critT('Formato PDF', ['PDF una página', 'Dos páginas', 'Otro formato', 'Nada'])] }
const R_SOLO_TIEMPO = { tipo: 'rubrica', niveles: NIV_T, criterios: [critT('Composición de dispositivos'), critT('ENTREGA A TIEMPO', ['', '', '', ''])] }
const R_MIXTO = { tipo: 'rubrica', niveles: NIV_T, criterios: [
  critT('Composición de dispositivos', ['Red completa', 'Casi', 'Incompleta', 'Nada']),
  critT('Cumplimiento de formato y fecha límite', ['Archivo entregado en PDF antes de la fecha límite establecida.', 'Archivo en PDF entregado en la fecha límite exacta.', 'Archivo en PDF entregado después de la fecha límite.', 'Archivo no está en formato PDF o no fue entregado.']),
] }
const C_NORMAL = { tipo: 'cotejo', niveles: [{ nombre: 'Nivel de desempeño', porcentaje: 100 }], criterios: [{ nombre: 'Esquema completo', puntos: [6] }, { nombre: 'FIRMADO', puntos: [4] }] }
const C_TIEMPO = { tipo: 'cotejo', niveles: [{ nombre: 'Nivel de desempeño', porcentaje: 100 }], criterios: [{ nombre: 'ENTREGA A TIEMPO', puntos: [4], descriptores: [''] }, { nombre: 'FIRMADO', puntos: [6] }] }

// Cliente falso: captura el prompt y responde lo que se le pida — incluido un
// nivel para el criterio de tiempo, que el código debe descartar.
const promptsT = []
const clienteT = (respuesta) => ({ messages: { create: async (req) => {
  promptsT.push({ system: req.system, texto: req.messages[0].content[0].text })
  return { content: [{ type: 'text', text: JSON.stringify(respuesta) }], usage: {} }
} } })
const evalT = (rubrica, respuesta) => FIA.evaluarEntregaConIA({
  client: clienteT(respuesta), modelo: 'x', rubrica, nombre: 'Red LAN', instrucciones: 'Construye la red y expórtala a PDF',
  nombreEstudiante: 'Ana', evidenciasBloques: [], evidenciasDetalle: [],
})
const todoCeroT = (n) => ({ criterios: Array.from({ length: n }, (_, i) => ({ n: i + 1, nivel: 0, evidencia: 'ok', sinEvidenciaSuficiente: false })), retroalimentacionGeneral: 'Bien', confianza: 'alta' })

const rNormal = await evalT(R_NORMAL, todoCeroT(2))
const rSolo = await evalT(R_SOLO_TIEMPO, todoCeroT(2))
const rMixto = await evalT(R_MIXTO, todoCeroT(2))
const rCotT = await evalT(C_TIEMPO, todoCeroT(2))
const rCotN = await evalT(C_NORMAL, todoCeroT(2))

caso('1. rúbrica sin criterio de tiempo: se evalúa normal y el total se propone', () => {
  assert.deepStrictEqual(rNormal.sugerencia.criterios.map((c) => c.nivel), [0, 0])
  assert.ok(rNormal.sugerencia.criterios.every((c) => !c.criterioDeTiempo))
  assert.strictEqual(FIA.totalPropuestaIA(R_NORMAL, rNormal.sugerencia.criterios), 10)
  assert.ok(promptsT[0].texto.includes('"Formato PDF"') && promptsT[0].texto.includes('nivel 0 (Excelente): Red completa'))
})

caso('2. criterio exclusivamente de tiempo: queda null con el aviso y no hay total', () => {
  const c = rSolo.sugerencia.criterios[1]
  assert.strictEqual(c.nivel, null)
  assert.strictEqual(c.criterioDeTiempo, true)
  assert.strictEqual(c.evidencia, FIA.AVISO_CRITERIO_TIEMPO)
  assert.strictEqual(rSolo.sugerencia.criterios[0].nivel, 0, 'el criterio académico sí se evalúa')
  assert.strictEqual(FIA.totalPropuestaIA(R_SOLO_TIEMPO, rSolo.sugerencia.criterios), null)
})

caso('3. criterio mixto formato + fecha: pendiente COMPLETO, sin evaluar el formato; los demás sí', () => {
  assert.strictEqual(rMixto.sugerencia.criterios[1].nivel, null)
  assert.strictEqual(rMixto.sugerencia.criterios[1].criterioDeTiempo, true)
  assert.strictEqual(rMixto.sugerencia.criterios[0].nivel, 0)
  assert.strictEqual(FIA.totalPropuestaIA(R_MIXTO, rMixto.sugerencia.criterios), null)
  // al modelo no le llega el texto del criterio (ni "fecha límite")
  const p = promptsT[2].texto
  assert.doesNotMatch(p, /fecha l[ií]mite|Cumplimiento de formato/i)
  assert.match(p, /2\. \[CRITERIO RESERVADO AL DOCENTE/)
})

caso('4. detecta criterios de tiempo por nombre o descriptor', () => {
  const t = (nombre, descriptores) => FIA.esCriterioDeTiempo({ nombre, descriptores })
  assert.ok(t('ENTREGA A TIEMPO'))
  assert.ok(t('Entrega', ['Entrega en tiempo y forma.', 'Entrega con poco retraso.', 'Entrega fuera de tiempo.']))
  assert.ok(t('Formato de entrega y cumplimiento', ['Entrega en PDF antes de la fecha límite', 'PDF pero con retrasos menores']))
  assert.ok(t('Puntualidad'))
  assert.ok(t('Cumplimiento del plazo'))
  assert.ok(t('Respeta la fecha de entrega'))
  assert.ok(t('Entrega', ['Entrega tardía']))
  assert.ok(t('Entrega', ['Entregó tarde']))
  assert.ok(t('Entrega extemporánea'))
})

caso('5. falsos positivos: contenidos académicos con fechas/tiempo NO son criterio de tiempo', () => {
  const t = (nombre, descriptores = []) => FIA.esCriterioDeTiempo({ nombre, descriptores })
  assert.ok(!t('Ubica las fechas históricas', ['Identifica todas las fechas históricas clave']))
  assert.ok(!t('Línea del tiempo', ['Ordena los sucesos en la línea del tiempo']))
  assert.ok(!t('Sistemas en tiempo real'))
  assert.ok(!t('Inversión a largo plazo'))
  assert.ok(!t('Composición de dispositivos'))
  assert.ok(!t('Presentación', ['Tómate el tiempo de revisar la ortografía']))
})

caso('6. cotejo con indicador de tiempo: pendiente (ni CUMPLE ni NO CUMPLE marcado por la IA) y sin total', () => {
  const [tiempo, firmado] = rCotT.sugerencia.criterios
  assert.strictEqual(tiempo.nivel, null)
  assert.strictEqual(tiempo.criterioDeTiempo, true)
  assert.strictEqual(firmado.nivel, 0)
  assert.strictEqual(FIA.totalPropuestaIA(C_TIEMPO, rCotT.sugerencia.criterios), null)
  assert.strictEqual(CAP.tieneCriterioDeTiempoPendiente(rCotT.sugerencia), true, '"Aplicar a todas" la salta')
})

caso('7. cotejo sin indicador de tiempo: comportamiento de siempre (total aunque haya un NO CUMPLE)', () => {
  assert.strictEqual(FIA.totalPropuestaIA(C_NORMAL, rCotN.sugerencia.criterios), 10)
  const conNoCumple = [{ n: 1, nivel: 0 }, { n: 2, nivel: null, sinEvidenciaSuficiente: true }]
  assert.strictEqual(FIA.totalPropuestaIA(C_NORMAL, conNoCumple), 6)
  assert.strictEqual(CAP.tieneCriterioDeTiempoPendiente({ criterios: conNoCumple }), false)
})

caso('8. ningún prompt de calificación lleva fechas y el sistema prohíbe usarlas', () => {
  for (const { system, texto } of promptsT) {
    assert.doesNotMatch(texto, /\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}\/\d{4}|fecha l[ií]mite|fecha de entrega/i)
    assert.match(system, /REGLA ESTRICTA sobre FECHAS/)
  }
  // la regla está en el sistema, no en el prompt: la rúbrica normal lleva sus criterios tal cual
  assert.ok(promptsT[0].texto.includes('RÚBRICA — niveles disponibles (0=Excelente, 1=Bueno, 2=Satisfactorio, 3=Insuficiente'))
})

caso('9. aunque el modelo devuelva nivel para un criterio de tiempo, el código lo anula', () => {
  // todoCeroT pidió nivel 0 en TODOS los criterios, incluidos los de tiempo
  for (const r of [rSolo, rMixto, rCotT]) {
    const t = r.sugerencia.criterios.filter((c) => c.criterioDeTiempo)
    assert.ok(t.length === 1 && t[0].nivel === null && t[0].sinEvidenciaSuficiente === true)
    assert.doesNotMatch(t[0].evidencia, /^ok$/)
  }
})

caso('la rúbrica del docente no se modifica al evaluar', () => {
  assert.strictEqual(R_MIXTO.criterios[1].nombre, 'Cumplimiento de formato y fecha límite')
  assert.strictEqual(R_MIXTO.criterios[1].descriptores[0], 'Archivo entregado en PDF antes de la fecha límite establecida.')
  assert.ok(!('criterioDeTiempo' in R_MIXTO.criterios[1]))
})
