...
//
// Función PURA a propósito, sin imports que dependan de un bundler: separa
// QUÉ va en el reporte de CÓMO se dibuja con jsPDF (eso vive en pdf.js, cuyos
// imports relativos sin extensión no resuelven bajo Node puro, así que
// ninguna función de pdf.js se puede probar directo con
...
//
// Todos los números (porcentajes, ranking de reactivos, confiabilidad) vienen
// de `resultado`, que ya los calculó Evalúa Fácil (ver functions/ia.js,
// agregarResultados) — este archivo no inventa ni recalcula nada, solo
// acomoda lo que ya existe para imprimirlo.
import { resumenConfiabilidad } from './confiabilidadAnalisis.js'
...
export function contenidoAnalisisResultadosPDF({ activity, subject, resultado, generadoEn = null, membrete = null }) {
  return {
    ...
    // Análisis de antes de esta corrección no traen `confiabilidad` —
    // `resumenConfiabilidad` devuelve null en ese caso y el PDF simplemente
    // no imprime la sección (nunca se inventa retroactivamente).
    textoConfiabilidad: resumenConfiabilidad(resultado?.confiabilidad),
