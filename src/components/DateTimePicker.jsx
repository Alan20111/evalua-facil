...
function TimeCol({ items, selected, onSelect, format, label }) {
  const selRef = useRef()
  const colRef = useRef()
        ...
        <div className="absolute left-0 mt-1 z-50 bg-surface-card border border-outline-variant rounded-xl shadow-2xl overflow-hidden"
          ...
          <div className="flex gap-2 px-3 py-2 border-t border-outline-variant">
            ...
            <button type="button" onClick={handleConfirm}
              className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-accent text-white text-xs font-semibold rounded-lg hover:bg-accent/90 transition-colors">
              <Check size={13} />
              Confirmar
            </button>
          </div>
