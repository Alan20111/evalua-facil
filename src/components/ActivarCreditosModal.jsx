// Activación voluntaria de los 50 créditos de bienvenida (20-ago-2026,
...
export default function ActivarCreditosModal({ open, onClose, onSuccess }) {
  ...
  async function activar() {
    ...
    try {
      ...
      toast('¡Listo! Ya tienes 50 créditos IA disponibles.')
      onSuccess?.()
      onClose()
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Activa tus 50 créditos IA de regalo" variant="centered" size="sm">
      <div className="space-y-4 text-center">
        <Gift className="mx-auto text-accent" size={40} />
        <p className="text-sm text-on-surface">
          Recibe 50 créditos de IA gratis para probar las funciones inteligentes de Evalúa Fácil.
        </p>
        ...
        >
          ...
          {activando ? 'Activando…' : 'Activar mis 50 créditos'}
        </button>
      </div>
    </Modal>
  )
